...
// template<typename T>
// T My_interpolated(float alpha,float beta,float gamma,T res){
//     float w_reciprocal = 1.0/(alpha  + beta  + gamma );
//     return alpha * res[0] + beta * res[1] + gamma * res[2];
// }
...
{
    auto v = t.toVector4();
    int start_x = std::min(std::min(int(t.v[0].x()),int(t.v[1].x())),int(t.v[2].x()));
    start_x = std::max(0,start_x);

    int end_x = std::max(std::max(int(t.v[0].x() + 1),int(t.v[1].x() + 1)),int(t.v[2].x() + 1));
    end_x = std::min(width - 1,end_x);

    int start_y = std::min(std::min(int(t.v[0].y()),int(t.v[1].y())),int(t.v[2].y()));
    start_y = std::max(0,start_y);

    int end_y = std::max(std::max(int(t.v[0].y() + 1),int(t.v[1].y() + 1)),int(t.v[2].y() + 1));
    end_y = std::min(height - 1,end_y);
    float dx[] = {0,0,-0.5,0.5};
    float dy[] = {0.5,-0.5,0,0};
        ...
        for (int j = start_y;j <= end_y;j++){
            ...
            if (insideTriangle(i + 0.5,j + 0.5,t.v)){
                ...
                if (z_interpolated < depth_buf[get_index(i,j)]){
                    ...
                    // interpolated_texcoords = interpolated_texcoords * w_reciprocal;
                    interpolated_texcoords *= w_reciprocal;

                    Vector3f interpolated_shadingcoords = alpha * view_pos[0] + beta * view_pos[1] + gamma * view_pos[2];
                    // interpolated_shadingcoords = interpolated_shadingcoords * w_reciprocal;
                    interpolated_shadingcoords *= w_reciprocal;

                    fragment_shader_payload payload( interpolated_color, interpolated_normal.normalized(), interpolated_texcoords, texture ? &*texture : nullptr);
                    payload.view_pos = interpolated_shadingcoords;
                    // auto pixel_color = float(cnt) / 4.0f * fragment_shader(payload) + float(4 - cnt)/ 4.0f * frame_buf[get_index(i,j)];
